...
def eval_NN_performance(node_per_layer, num_layer, data, max_train_iter, N_u):
    # setting of NN Arch.
...
if __name__ == "__main__":
    ...
    num_layer = 8
    N_u = [500]
    #N_u = list([50, 100, 200, 300, 400, 456])
    for i in N_u:
        error = list([])
        for j in range(10):
            np.random.seed(i+j)
            cur_err = eval_NN_performance(node_per_layer, num_layer, data, max_iter, i)
