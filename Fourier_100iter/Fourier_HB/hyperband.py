...
class HyperBand:
    ...
    def __init__(self, model, max_iter=50000, eta=np.e):
    ...
    def run(self):
        for s in reversed(range(self.s_max + 1)):
            ...
            print(configs)
            f = open("./cmd_int.txt", "a")
            #t = open("./dw_cmd_float.txt", "a")
            for config in configs:
                f.write('python run.py %d %d\n' % (int(np.round(config['node_per_layer'])),
                                                      int(np.round(config['num_layer']))))
