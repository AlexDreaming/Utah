...
plt.text(10, -3.0, 'Depth = 10', fontsize=18, ha='left')

cm = plt.cm.get_cmap('RdYlBu_r')
sc = plt.scatter(width_sort, np.log10(pair[:, 1]), c=pair[:, 3], marker='s', s=100, cmap=cm)
cbar = plt.colorbar(sc)
cbar.set_label('Iter', size=15)
max = int(np.argmax(pair[:, 3]))
plt.scatter(width_sort[max], np.log10(pair[:, 1][max]), color='', marker='s', edgecolors='#AD03DE', s=800)
