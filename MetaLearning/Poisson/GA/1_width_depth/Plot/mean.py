...
fig, ax = plt.subplots()
norm = colors.Normalize(vmax=-3.5, vmin=-3.9)

cm = plt.cm.get_cmap('RdYlBu')

sc = ax.scatter(depth, width, s=75, c=np.log10(mean), norm=norm, alpha=0.8, cmap=cm)
ax.set_xticks(np.arange(2, 9, 1))
ax.set_yticks(np.arange(20, 65, 5))
cbar = plt.colorbar(sc)
cbar_range = np.arange(-3.9, -3.45, 0.05)
cbar.set_ticks(cbar_range)
labels = np.round(np.power(10, cbar_range), 5)
cbar.set_ticklabels(labels)
cbar.set_label('Mean', size=15)
